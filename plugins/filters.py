...
async def addfilter(client, message):
    ...
    if (len(extracted) >= 2) and not message.reply_to_message:
        ...
        try:
            ...
            reply_text = message.reply_to_message.caption.html if msg else message.reply_to_message.text.html
            btn = message.reply_to_message.reply_markup.inline_keyboard if message.reply_to_message.reply_markup else "[]"
            alert = None
        except:
            reply_text, btn, fileid, alert = "", "[]", None, None
    else:
        return
...
async def advantage_spell_chok(msg):
    ...
    # Extract movie request safely
    mv_rqst = " ".join(msg.text.split()[1:]).strip()  # Ensures proper handling of spaces
    ...
    try:
        reqst_gle = mv_rqst.replace(" ", "+")  # Fix scope issue
        movies = await get_poster(reqst_gle, bulk=True)  # Ensure `get_poster` is imported
